...
$logfile=$CurrentDir + "\Log_Install_HSC_Intranet.log"
...
Write-Host '- Deactivating features :' -foregroundcolor Yellow 

	Write-Host 'Powershell Script will now disable feature: Archetypes' -foregroundcolor Cyan 
	Disable-SPFeature -Identity $f_M_Archetypes -Url $RootSiteUrl -Force -Confirm:$false

	Write-Host 'Powershell Script will now disable feature: Lists definitions' -foregroundcolor Cyan 
	Disable-SPFeature -Identity $f_M_ListDefinition -Url $RootSiteUrl -Force -Confirm:$false

	Write-Host 'Powershell Script will now disable feature: Style library' -foregroundcolor Cyan 
	Disable-SPFeature -Identity $f_M_StyleLibrary -Url $RootSiteUrl -Force -Confirm:$false

	Write-Host 'Powershell Script will now disable feature: Master pages' -foregroundcolor Cyan 
	Disable-SPFeature -Identity $f_M_MaterPages -Url $RootSiteUrl -Force -Confirm:$false

	Write-Host 'Powershell Script will now disable feature: Create sites' -foregroundcolor Cyan 
	Disable-SPFeature -Identity $f_M_CreateSites -Url $RootSiteUrl -Force -Confirm:$false

		if($InstallSite_RootSite -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - Root Site' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_RootSite -Url $RootSiteUrl -Force -Confirm:$false
		}
		if($InstallSite_ApplicationInformatique -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - Applications informatiques' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_ApplicationInformatique -Url $SiteUrl_ApplicationInformatique -Force -Confirm:$false
		}
		if($InstallSite_Bottin -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - Bottin' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_Bottin -Url $SiteUrl_Bottin -Force -Confirm:$false
		}
		if($InstallSite_Communication -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - Communication' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_Communication -Url $SiteUrl_Communication -Force -Confirm:$false
		}
		if($InstallSite_DocumentsAdministratifs -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - Documents Administratifs' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_DocumentsAdministratifs -Url $SiteUrl_DocumentsAdministratifs -Force -Confirm:$false
		}
		if($InstallSite_Organisation -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - Organization' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_Organisation -Url $SiteUrl_Organisation -Force -Confirm:$false
		}
		if($InstallSite_ReferencesClinique -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - References cliniques' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_ReferencesClinique -Url $SiteUrl_ReferencesClinique -Force -Confirm:$false
		}
		if($InstallSite_VieAuTravail -eq $True)
		{
		Write-Host 'Powershell Script will now disable feature: Site - Vie au travail' -foregroundcolor Cyan 
		Disable-SPFeature -Identity $f_S_VieAuTravail -Url $SiteUrl_VieAuTravail -Force -Confirm:$false
		}

Write-Host '- Uninstall solution :' -foregroundcolor Yellow 
...
Write-Host '- Remove solution :' -foregroundcolor Yellow 
	Remove-SPSolution -Identity $solutionName -Force -Confirm:$false

Write-Host '- Add solution :' -foregroundcolor Yellow 
...
Write-Host '- Activating features' -foregroundcolor Yellow 
	...
	Enable-SPFeature -Identity $f_M_CreateSites -Url $RootSiteUrl -Force -Confirm:$false
		...
		if($InstallSite_Organisation -eq $True)
		{
		Write-Host 'Powershell Script will now enable feature: Site - Organization' -foregroundcolor Cyan 
